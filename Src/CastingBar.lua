function BananaTargetCastingBar_OnLoad(self)
    ...
    self:RegisterEvent('PLAYER_TARGET_CHANGED') -- 需要
    self.unit = 'target'
...
function BananaTargetCastingBar_OnShow(self)
...
function BananaTargetCastingBar_OnEvent(self, event, ...)
    local arg1 = ...
    ...
    if (event == 'PLAYER_ENTERING_WORLD' or event == 'PLAYER_TARGET_CHANGED') then
        ...
        elseif (nameSpell) then
            ...
            -- 替换自 TargetFrame.lua:885，需要处理PLAYER_TARGET_CHANGED，直接隐藏而不是渐隐
			self.casting = nil;
			self.channeling = nil;
...
function BananaTargetCastingBar_OnUpdate(self, elapsed)
    ...
    if (self.casting) then
        ...
        if (self.value >= self.maxValue) then
            self:SetValue(self.maxValue)
            BananaTargetCastingBar_FinishSpell(self, barSpark, barFlash)
            return
        end
        self:SetValue(self.value)
        if (barFlash) then
...
function BananaTargetCastingBar_FinishSpell(self, barSpark, barFlash)
    self:SetStatusBarColor(0.0, 1.0, 0.0)
    if (barSpark) then
        barSpark:Hide()
    end
...
function BananaTargetCastingBar_UpdateIsShown(self)
    if (self.casting and self.showCastbar) then
        BananaTargetCastingBar_OnEvent(self, 'PLAYER_ENTERING_WORLD')
    else
        self:Hide()
    end
end
